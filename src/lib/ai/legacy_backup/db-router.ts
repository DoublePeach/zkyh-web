/**
 * @description 数据库与AI模型连接服务，基于PostgreSQL MCP Server
 * @author 郝桃桃
 * @date 2024-08-05
 */
 ...
 */
async function fetchChapters(examSubjectId: string): Promise<Chapter[]> {
  try {
    // 使用OpenAI调用PostgreSQL MCP Server
    const openai = new OpenAI({
      apiKey: AI_CONFIG.OPENROUTER_API_KEY,
...
export async function generateStudyPlanFromDatabase(surveyData: SurveyFormData): Promise<AIResponseData> {
  try {
    ...
    const subjectsWithChapters = await Promise.all(
      examSubjects.map(async (subject: ExamSubject) => {
